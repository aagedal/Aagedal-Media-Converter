...
final class AppIntentHandoffTests: XCTestCase {
    ...
    func testSharedBridgeCapturesSaveNextToOriginalDestinations() throws {
        let defaults = try makeIsolatedDefaults()
        ...
        let firstSource = URL(fileURLWithPath: "/sources/one/first.mov")
        let secondSource = URL(fileURLWithPath: "/sources/two/second.mov")
        ...
        XCTAssertEqual(request.sourceSettings?.map(\.includeDateTag), [true, true])
        XCTAssertEqual(request.sourceSettings?.map(\.timecodeMode), [.preserveSource, .preserveSource])
    ...
    @MainActor
    func testSharedBridgePersistsOnlyPerSourceIntentDestinations() throws {
        let defaults = try makeIsolatedDefaults()
        ...
        AppIntentApplicationJobBridge.persistFileAccess(for: request, bookmarks: bookmarks)
